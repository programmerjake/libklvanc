...
static int cb_VANC_TYPE_KL_UINT64_COUNTER(void *callback_context,
    struct klvanc_context_s *ctx, struct klvanc_packet_kl_u64le_counter_s *pkt)
{
	printf("%s:%s()\n", __FILE__, __func__);
	return 0;
}
...
*/
	unsigned short arr[] = {
		...
		0x06, 0x0a, 0x2b, 0x34, 0x01, 0x01, 0x01, 0x05, 0x01, 0x01, 0x0d, 0x43,
		0x33, 0x00, 0x00, 0x00, 0xe6, 0x8a, 0x1e, 0x00, 0x43, 0x62, 0x05, 0x80,
		0x08, 0x00, 0x46, 0x02, 0x01, 0x20, 0x08, 0xa4, 0x87, 0x20, 0xa3, 0x07,
		0x43, 0x62, 0x05, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	...
	int ret =
	    klvanc_packet_parse(ctx, 17, arr, sizeof(arr) / (sizeof(unsigned short)));
	if (ret < 0)
		return ret;
...
*/
	...
	unsigned short arr[] = {
		0x000, 0x3ff, 0x3ff, 0x162, 0x101, 0x11A, /* DC */
		0x0ad, /* Payload Field = "ATSC private information descriptor" */
		0x018, /* Payload length */
		0x053, 0x045, 0x04e, 0x031, 0x001, 0x057, 0x053, 0x045, 0x045, 0x020,
		0x054, 0x056, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000,
		0x001, 0x0f0, 0x004, 0x001, 0x24e, /* Checksum is valid */
	};

	/* report that this was from line 13, informational only. */
	int ret =
	    klvanc_packet_parse(ctx, 13, arr, sizeof(arr) / (sizeof(unsigned short)));
	if (ret < 0)
		return ret;
...
*/
	...
	unsigned short arr2[] = {
		0x000, 0x3ff, 0x3ff, 0x162, 0x101, 0x117, /* DC */
		0x0ad, /* Payload Field = "ATSC private information descriptor" */
		0x015, /* Payload length */
		0x053, 0x045, 0x04e, 0x031, 0x004, 0x000, 0x000, 0x000, 0x000, 0x000,
		0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x000, 0x001,
		0x034, 0x18c, /* Checksum is valid */
	};
...
{
	unsigned short arr[] = {
		0x000, 0x3ff, 0x3ff, 0x241, 0x105, 0x108, 0x07c, /* Payload Field */
		0x000, /* Payload Field */
		0x000, /* Payload Field */
		0x000, /* Payload Field */
		0x000, /* Payload Field */
...
static int test_EIA_708B(struct klvanc_context_s *ctx)
{
	...
	int ret =
	    klvanc_packet_parse(ctx, 1, arr, sizeof(arr) / (sizeof(unsigned short)));
	if (ret < 0)
		return ret;
...
static unsigned char eia608_entry[] = { 0x00, 0x00, 0x03, 0xff, 0x03, 0xff, 0x01, 0x61,
	0x01, 0x02, 0x02, 0x03, 0x02, 0x95, 0x01, 0x80, 0x01, 0x80, 0x01, 0xfb, 0x02,
	0x00, 0x00, 0x40, 0x02, 0x00 };
static int test_EIA_608(struct klvanc_context_s *ctx)
{
	...
	int ret = klvanc_packet_parse(
	    ctx, 13, arr, sizeof(eia608_entry) / sizeof(unsigned short));
	free(arr);
...
static int test_checksum()
{
	...
	unsigned short arr[] = { 0x0000, 0x03ff, 0x03ff, 0x0241, 0x0107, 0x011c, 0x0108,
		0x0200, 0x0101, 0x0200, 0x021b, 0x02ff, 0x02ff, 0x02ff, 0x02ff, 0x0200,
		0x0200, 0x0200, 0x0200, 0x0200, 0x0104, 0x0200, 0x0200, 0x0200, 0x0102,
		0x0200, 0x0101, 0x0200, 0x0200, 0x0200, 0x0200, 0x0200, 0x0200, 0x0101 };

	uint16_t sum = klvanc_checksum_calculate(&arr[3], 31);
	if (sum != 0x28c)
		return -1;

	unsigned short arr2[] = { 0x0000, 0x03ff, 0x03ff, 0x0241, 0x0107, 0x011c, 0x0108,
		0x0200, 0x0101, 0x0200, 0x021b, 0x02ff, 0x02ff, 0x02ff, 0x02ff, 0x0200,
		0x0200, 0x0200, 0x0200, 0x0200, 0x0104, 0x0200, 0x0200, 0x0200, 0x0102,
		0x0200, 0x0101, 0x0200, 0x0200, 0x0200, 0x0200, 0x0200, 0x0200, 0x0101,
		0x028c };
...
static unsigned char __0_vancentry[] = {  };
...
static int test_scte_104(struct klvanc_context_s *ctx)
{
	...
	int ret = klvanc_packet_parse(
	    ctx, 13, arr, sizeof(__0_vancentry) / sizeof(unsigned short));
	free(arr);
...
static int test_multimessage(struct klvanc_context_s *ctx)
{
	...
	uint16_t arr[] = {
		/* Msg 1 */
		0x0000, 0xff03, 0xff03, 0x4102, 0x0701, 0xff02, 0x0c02,
		0xff02, // | ......A.........
		0xff02, 0x0101, 0x0e01, 0x0002, 0x0002, 0x0902, 0x0002,
		0x0002, // | ................
		0x0002, 0x0002, 0x0502, 0x0101, 0x0401, 0x0002, 0x0201,
		0x0f02, // | ................
		0xa002, 0x0101, 0x0b01, 0x0002, 0x2301, 0x0701, 0xb802,
		0x4202, // | ........#.....B.
		0xb901, 0x0002, 0x0002, 0x0002, 0x0101, 0x1102, 0x5502,
		0x4b02, // | ............U.K.
		0x5201, 0x4102, 0x4901, 0x4e02, 0x4501, 0x4802, 0x4501,
		0x4c01, // | R.A.I.N.E.H.E.L.
		0x5002, 0x3002, 0x3401, 0x5701, 0x5002, 0x5602, 0x4901,
		0x3502, // | P.0.4.W.P.V.I.5.
		0x0101, 0x0101, 0x0002, 0x0002, 0x0101, 0x0101, 0x0101,
		0x0302, // | ................
		0x0101, 0x0b01, 0x0002, 0x4301, 0x0701, 0x8202, 0xab01,
		0x6902, // | ......C.......i.
		0x0002, 0x0002, 0x0002, 0x0101, 0x3101, 0x5002, 0x4301,
		0x5201, // | ........1.P.C.R.
		0x3101, 0x5f02, 0x3002, 0x3302, 0x3002, 0x3502, 0x3201,
		0x3201, // | 1._.0.3.0.5.2.2.
		0x3002, 0x3502, 0x3502, 0x3801, 0x5701, 0x5002, 0x5602,
		0x4901, // | 0.5.5.8.W.P.V.I.
		0x4102, 0x4301, 0x5401, 0x4901, 0x4f01, 0x4e02, 0x4e02,
		0x4501, // | A.C.T.I.O.N.N.E.
		0x5701, 0x5302, 0x5302, 0x4102, 0x5401, 0x5502, 0x5201,
		0x4402, // | W.S.S.A.T.U.R.D.
		0x4102, 0x5902, 0x4d02, 0x4f01, 0x5201, 0x4e02, 0x4901,
		0x4e02, // | A.Y.M.O.R.N.I.N.
		0x4702, 0x4102, 0x5401, 0x3602, 0x4102, 0x4d02, 0x1102,
		0x0101, // | G.A.T.6.A.M.....
		0x0101, 0x0002, 0x0002, 0x0101, 0x0101, 0x0101, 0x0302,
		0x0101, // | ................
		0x0b01, 0x0002, 0x4301, 0x0701, 0xb901, 0xb202, 0x1601,
		0x0002, // | ....C...........
		0x0002, 0x0002, 0x0101, 0x3101, 0x5002, 0x4301, 0x5201,
		0x3101, // | ......1.P.C.R.1.
		0x5f02, 0x3002, 0x3302, 0x3002, 0x3502, 0x3201, 0x3201,
		0x3002, // | _.0.3.0.5.2.2.0.
		0x3602, 0x3502, 0x3801, 0x5701, 0x5002, 0x5602, 0x4901,
		0x4102, // | 6.5.8.W.P.V.I.A.
		0x4301, 0x5401, 0x4901, 0x4f01, 0x4e02, 0x4e02, 0x4501,
		0x5701, // | C.T.I.O.N.N.E.W.
		0x5302, 0x5302, 0x4102, 0x5401, 0x5502, 0x5201, 0x4402,
		0x4102, // | S.S.A.T.U.R.D.A.
		0x5902, 0x4d02, 0x4f01, 0x5201, 0x4e02, 0x4901, 0x4e02,
		0x4702, // | Y.M.O.R.N.I.N.G.
		0x4102, 0x5401, 0x3701, 0x4102, 0x4d02, 0x1001, 0x0101,
		0x0101, // | A.T.7.A.M.......
		0x0002, 0x0002, 0x0101, 0x0101, 0x0101, 0x0302, 0x0101,
		0x0b01, // | ................
		0x0002, 0x4301, 0x0701, 0xb901, 0xb202, 0x1702, 0x0002,
		0x0201, // | ..C.............
		0x6201, 0x0101, 0x3101, 0x5002, 0x4301, 0x5201, 0x3101,
		0x5f02, // | b...1.P.C.R.1._.
		0x3002, 0x3302, 0x3002, 0x3502, 0x3201, 0x3201, 0x3002,
		0x3602, // | 0.3.0.5.2.2.0.6.
		0x3502, 0x3801, 0x5701, 0x5002, 0x5602, 0x4901, 0x4102,
		0x4301, // | 5.8.W.P.V.I.A.C.
		0x5401, 0x4901, 0x4f01, 0x4e02, 0x4e02, 0x4501, 0x5701,
		0x5302, // | T.I.O.N.N.E.W.S.
		0x5302, 0x4102, 0x5401, 0x5502, 0x5201, 0x4402, 0x4102,
		0x5902, // | S.A.T.U.R.D.A.Y.
		0x4d02, 0x4f01, 0x5201, 0x4e02, 0x4901, 0xd502, // | M.O.R.N.I...

		/* Msg 2 */
		0x0000, 0xff03, 0xff03, 0x4102, 0x0701, 0x1102, 0x0a02,
		0x4e02, // | ......A.......N.
		0x4702, 0x4102, 0x5401, 0x3701, 0x4102, 0x4d02, 0x2001,
		0x0101, // | G.A.T.7.A.M. ...
		0x0101, 0x0002, 0x0002, 0x0101, 0x0101, 0x0101, 0x0302,
		0x7a01, // | ..............z.
	};
