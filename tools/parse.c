...
static void convert_colorspace_and_parse_vanc(
    unsigned char *buf, unsigned int uiWidth, unsigned int lineNr)
{
	...
	if (klvanc_v210_line_to_nv20_c(
		src, p_anc, sizeof(decoded_words), (uiWidth / 6) * 6) < 0)
		return;

	int ret = klvanc_packet_parse(vanchdl, lineNr, decoded_words,
...
static int AnalyzeVANC(const char *fn)
{
	...
	while (!feof(fh)) {
		...
		if (g_filterMatch) {
			...
			if (vancOutputFile) {
				...
				fwrite(
				    &uiHeight, sizeof(unsigned int), 1, vancOutputFile);
				fwrite(
				    &uiStride, sizeof(unsigned int), 1, vancOutputFile);
	...
	if (g_filter_did || g_filter_sdid) {
		if (g_filtermatchCount == 0)
			fprintf(stderr,
			    "Filtering requested but no matching records found\n");
		else
    ...
    struct klvanc_packet_header_s *pkt)
{
	if (pkt_filtered(pkt)) {
		...
		if (g_saveVanc > 0) {
			char tmpfname[256];
			snprintf(
			    tmpfname, sizeof(tmpfname), "%d.vancentry", g_vancEntryCount);
...
static int usage(const char *progname, int status)
{
	...
	fprintf(
	    stderr, "Usage: %s -m <mode id> [OPTIONS]\n", basename((char *) progname));
	...
	fprintf(stderr,
	    ...
	    basename((char *) progname), basename((char *) progname));
...
int parse_main(int argc, char *argv[])
{
	...
	while ((ch = getopt(argc, argv, "?hf:o:p:vxI:d:s:")) != -1) {
		switch (ch) {
		case 'o': g_vancOutputFilename = optarg; break;
		case 'I': g_vancInputFilename = optarg; break;
		case 'd':
		...
		case 'x': g_saveVanc++; break;
		case '?':
		case 'h': wantHelp = true;
		}
	}
